...
import {
  delegateSubject,
  removeSubjectDelegation,
  replaceSubjectDelegation
} from '@/api/subject-api'
import {
  delegateCategory,
  removeCategoryDelegation,
  replaceCategoryDelegation
...
export default {
  ...
  computed: {
    ...mapGetters(['collaborator', 'collaborators', 'currentSubject', 'currentCategory', 'filter']),
    ...
    sorted() {
      return (this.filter.channel.collaborators || this.collaborators || [])
        ...
        .filter(
          collaborator =>
            collaborator.toLowerCase().indexOf(this.collaboratorFilter.toLowerCase()) !== -1 &&
            collaborator !== this.collaborator.email
        )
    },
  ...
  methods: {
    ...
    remove() {
      this.fetching = true
      return this.removeDelegation(this.dataId).then(() => {
        this.fetching = false
      })
    },
